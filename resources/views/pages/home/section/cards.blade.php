<div class="min-h-5/6 py-10 flex flex-col items-center justify-center bg-green-200">
    <div class="item flex flex-wrap justify-center gap-4">

        <!-- Card 1 -->
        <div data-aos="fade-up" data-aos-duration="1000" class="flex h-4/5 w-72 flex-col rounded-xl bg-white shadow-lg">
            <div class="h-[70%] rounded-t-xl text-center">
                <img src="{{ asset('image/photo.png') }}" alt="photo">
            </div>
            <div class="h-[30%] rounded-b-xl p-4 pt-0 text-center">
                <h1 class="text-2xl font-semibold text-green-600 font-serif">Scenery</h1>
                <p class="text-gray-700 text-sm">
                    Amidst nature, our resort offers scenic views and lush green surroundings—a peaceful blend of calm
                    and beauty.
                </p>
            </div>
        </div>

        <!-- Card 2 -->
        <div data-aos="fade-up" data-aos-duration="1000" class="flex h-4/5 w-72 flex-col rounded-xl bg-white shadow-lg">
            <div class="h-[70%] rounded-t-xl text-center">
                <img src="{{ asset('image/safe.png') }}" alt="photo">
            </div>
            <div class="h-[30%] rounded-b-xl p-4 pt-0 text-center">
                <h1 class="text-2xl font-semibold text-green-600 font-serif">Safety</h1>
                <p class="text-gray-700 text-sm">
                    Nestled in nature, our resort prioritizes safety. With well-lit paths and 24/7 surveillance.
                </p>
            </div>
        </div>

        <!-- Card 3 -->
        <div data-aos="fade-up" data-aos-duration="1000" class="flex h-4/5 w-72 flex-col rounded-xl bg-white shadow-lg">
            <div class="h-[70%] rounded-t-xl text-center">
                <img src="{{ asset('image/calm.png') }}" alt="photo">
            </div>
            <div class="h-[30%] rounded-b-xl p-4 pt-0 text-center">
                <h1 class="text-2xl font-semibold text-green-600 font-serif">Calmness</h1>
                <p class="text-gray-700 text-sm">
                    Experience serenity in our resort, where the rustle of gentle breezes. Embrace the lush
                    surroundings.
                </p>
            </div>
        </div>
    </div>

    <!-- Button -->
    <a data-aos="fade-up" data-aos-duration="1000" href="#next"
        class="inline-flex justify-center items-center mt-5 py-2 px-9 text-lg font-semibold text-center text-white rounded-full bg-green-600 border border-green-600 hover:bg-green-700 focus:ring-4 focus:ring-gray-400 shadow-md">
        About us
    </a>
</div>
