...
@foreach ($feedbacks as $feedback)
          ...
          <a data-aos="fade" data-aos-duration="900" class="relative mb-20 rounded-2xl bg-white text-center shadow dark:bg-gray-700">
            <div class="z-20 -mt-24 p-8">
              ...
              <div class="mb-3 inline-block h-32 w-32 overflow-hidden rounded-full bg-blue-500 text-xs text-white">
                <img class="h-full w-full object-cover transition-all hover:scale-110" src="{{ asset('storage/' . $feedback->image) }}" alt="" />
              </div>
              <p class="mb-4 text-base leading-7 text-gray-700">{{ $feedback->feedback }}</p>
              <h2 class="text-lg font-bold leading-9 text-black dark:text-white font">{{ $feedback->name }}</h2>
              <span class="block text-xs font-semibold uppercase text-green-600 dark:text-green-600">{{ $feedback->address }}</span>
